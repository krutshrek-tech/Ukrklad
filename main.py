...
@app.route('/')
def home():
    return "🤖 Telegram Bot Active | Operator: @kldukr_mng"
...
async def product_handler(update: Update, context):
    ...
    if data == "view_cart":
        ...
        order_text += "• Только ЛС с оператором\n"
        order_text += "• Не обсуждайте в чате\n"
        order_text += "• Оплата криптовалютой\n\n"
...
def run_bot():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            PRODUCT: [CallbackQueryHandler(product_handler, pattern='^(prod_|view_cart|finish)$')]
        },
...
if __name__ == '__main__':
    ...
    # Запускаем Flask сервер на порту 8000 (требуется Koyeb)
